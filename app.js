...
// ═══════════════════════════════════════════════════════════
// GITHUB PROJECTS — DYNAMIC PUBLIC REPOS
// ═══════════════════════════════════════════════════════════
const GITHUB_USER = 'bitzzdev';
const REPOS_CACHE_KEY = 'bitzdev_github_repos';
const REPOS_CACHE_TTL = 30 * 60 * 1000;
...
const REPO_PALETTES = [
    ...
    { colors: '#96c8ff,#EAB308', rotation: 45, speed: 0.2, frequency: 1.1, scale: 0.8 },
    { colors: '#22d3ee,#a855f7', rotation: 60, speed: 0.15, frequency: 1.3, scale: 0.7 },
    { colors: '#34d399,#22d3ee', rotation: 120, speed: 0.18, frequency: 1.4, scale: 0.85 },
    { colors: '#f472b6,#fb923c', rotation: 30, speed: 0.22, frequency: 1.5, scale: 0.75 }
];
...
function getCachedRepos() {
    try {
        ...
        if (Date.now() - data.fetchedAt > REPOS_CACHE_TTL) return null;
        return data.repos;
    } catch (e) {
        return null;
    }
...
function setCachedRepos(repos) {
    try {
        localStorage.setItem(REPOS_CACHE_KEY, JSON.stringify({ fetchedAt: Date.now(), repos }));
...
function buildRepoCard(repo, palette) {
    const link = document.createElement('a');
    link.href = repo.html_url;
    link.target = '_blank';
    link.rel = 'noopener noreferrer';
    link.className = 'work-item reveal';

    const bends = document.createElement('div');
    bends.className = 'color-bends';
    bends.dataset.colors = palette.colors;
    bends.dataset.rotation = palette.rotation;
    bends.dataset.speed = palette.speed;
    bends.dataset.intensity = palette.intensity;
    if (palette.frequency) bends.dataset.frequency = palette.frequency;
    if (palette.scale) bends.dataset.scale = palette.scale;

    const inner = document.createElement('div');
    inner.className = 'work-item-inner';
    ...
    desc.textContent = repo.description || repo.language || 'Open source project';
    ...
    arrow.innerHTML = '<svg width="16" height="16" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 17L17 7M17 7H7M17 7v10"></path></svg>';

    link.appendChild(bends);
...
async function loadGithubProjects() {
    ...
    if (!repos) {
        try {
            ...
            console.warn('[bitz.dev] Failed to load GitHub projects:', err);
            return;
        }
    }
    ...
    repos.forEach((repo, i) => {
        grid.appendChild(buildRepoCard(repo, REPO_PALETTES[i % REPO_PALETTES.length]));
    });

    initColorBends('.work-grid .color-bends');
